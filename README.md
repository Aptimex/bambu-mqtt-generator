...
The first argument is the command spec's name, which is the name Bambu Studio
gives the operation — not always the string that goes on the wire. The spec
above is `ams_filament_settings`, but it sends `"command": "ams_filament_setting"`.
Either name works; they select the same spec and build the same payload.

A wire name used by more than one spec can't select one on its own — three
specs send `print_option`, and three send `stop`. Those raise
`AmbiguousCommandNameError` (a `ValueError` subclass) listing the spec names to
choose from. `config/commands/` holds every spec name.
...
config.resolve_model_id("a1 mini")                                         # 'N1'
...
| `config/filament_presets/` | 103 filament presets |
